...
if (!process.env.ANTHROPIC_API_KEY) {
  ...
  console.error(
    '방법 1: 환경변수 설정 - export ANTHROPIC_API_KEY="sk-ant-..."'
  );
...
if (process.argv.length < 3) {
  ...
  console.log(
    "예시: npm run parse-identity-ai src/scripts/raw/meursault_data.txt"
  );
  console.log("📁 권장 경로:");
...
const __filename = fileURLToPath(import.meta.url); // ← URL→경로
...
const outputPath = path.join(outputDir, `${inputFileName}_converted.json`);
...
function saveAndValidateResult(result, outputPath) {
  ...
  console.log(
    "강화 스킬:",
    validation.summary.enhancementSkills.length > 0
      ? validation.summary.enhancementSkills.join(", ")
      : "없음"
  );
  ...
  if (result.data.sync3?.skill1) {
    ...
    if (skill.hit) {
      const hitCount = Object.values(skill.hit).filter(
        (v) => v && v.trim()
      ).length;
