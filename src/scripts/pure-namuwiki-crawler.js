  ...
  async handleCaptchaIfExists() {
    try {
      ...
      if (captchaFound) {
        ...
        console.log(
          '📱 "나는 로봇이 아닙니다"를 클릭하고 완료될 때까지 기다립니다.'
        );
    ...
    try {
      ...
      await this.page.waitForFunction(
        () => {
          ...
          // 기타 에디터 요소들
          const otherEditors = document.querySelector(
            ".inputarea, #text, .ace_text-input"
          );
          if (otherEditors) {
  ...
  async saveTextToFile(text, filename = null) {
    try {
      ...
      const timestamp = new Date()
        ...
        .replace(/[:.]/g, "-")
        .slice(0, -5);
      const safeName = personaName
        ? personaName.replace(/[<>:"\/\\|?*]/g, "_")
        : null;
      const finalFilename =
        filename ||
        (safeName ? `${safeName}.txt` : `namuwiki_extract_${timestamp}.txt`);
  ...
  async findSectionEditLink(sectionAnchor) {
    return await this.page.evaluate((sectionAnchor) => {
      ...
      while (node && node !== document.body) {
        const editLink = Array.from(
          node.querySelectorAll('a[href*="/edit/"]')
        ).find(
          (a) =>
      ...
      while (sibling) {
        if (
          sibling.matches('a[href*="/edit/"]') &&
          (sibling.textContent.includes("편집") ||
            sibling.getAttribute("rel") === "nofollow") &&
      ...
      const fallback = Array.from(
        document.querySelectorAll(`a[href*="/edit/"][href*="section="]`)
      ).find(
        (a) =>
          (a.textContent.includes("편집") ||
            a.getAttribute("rel") === "nofollow") &&
  ...
  async extractFromUrl(url, filename = null, sectionAnchor = null) {
    try {
      ...
      if (sectionAnchor) {
        ...
        if (!targetEditUrl) {
          throw new Error(
            `섹션(${sectionAnchor}) 편집 링크를 찾을 수 없습니다`
          );
        }
        console.log(
          `📝 섹션(${sectionAnchor}) 편집 페이지로 이동: ${targetEditUrl}`
        );
      } else {
        // 기존 전체 문서 편집 링크
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.log(
      '예시: npm run crawling "https://namu.wiki/w/...#s-2.3.7" "meursault_data"'
    );
    console.log("");
    console.log("💡 주의사항:");
  ...
  try {
    ...
    if (result && result.filePath) {
      try {
        ...
        execSync(`node "${parseScript}" "${result.filePath}"`, {
          stdio: "inherit",
        });
